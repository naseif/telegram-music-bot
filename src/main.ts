...
slimbot.on("message", async (message: any) => {
    ...
    if (StreamersRegexList.YOUTUBE.test(message.text)) {
        ...
        const keyboardOptionsArray = qualityOptions.filter(element => element !== undefined).map(option => new Array({ text: option, callback_data: option }))
...
slimbot.on('callback_query', async (query: any) => {
    if (qualityOptions.includes(query.data)) {
        ...
        if (query.data === "Audio") {
            await slimbot.sendMessage(query.message.chat.id, `Ok, Downloading ${searchResult.title} in ${query.data}, Please be patient`);
